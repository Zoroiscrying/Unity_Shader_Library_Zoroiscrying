#ifndef BASIC_DISSOLVE_PASS
#define BASIC_DISSOLVE_PASS
...
struct Varyings
{
    ...
    // Used for position based dissolve
...
//#define POSITION_BASED_DISSOLVE
//#define DISTANCE_BASED_DISSOLVE

float DissolveFactor_Position_Y_Noise(float positionCoord, float2 noiseCoord)
{
...
float DissolveFactor_Distance_Noise(float3 fragPositionCoord, float3 refPointCoord, float2 noiseCoord)
{
    float noiseDense = value_noise12(noiseCoord * _NoiseScale + _Time) * 2 - 1;
    float noiseLoose = value_noise12(noiseCoord * 4 + _Time) * 2 - 1;

    return distance(fragPositionCoord, refPointCoord) - _DistanceDissolveEdge + lerp(noiseDense, noiseLoose, 0.8) * _NoiseValueScaleFactor;
}

float DissolveFactor_UV_Noise(half2 uvCoord, float2 noiseCoord)
{
    float noiseDense = value_noise12(noiseCoord * _NoiseScale + _Time) * 2 - 1;
    float noiseLoose = value_noise12(noiseCoord * 4 + _Time) * 2 - 1;
    float noiseUV = value_noise12(uvCoord);
    return noiseUV - _UVDissolveEdge + lerp(noiseDense, noiseLoose, 0.8) * _NoiseValueScaleFactor;
}
...
half4 LitPassFragment(Varyings input) : SV_Target
{
...
#if defined(POSITION_BASED_DISSOLVE)
    dissolveFactor = DissolveFactor_Position_Y_Noise(input.positionOS.z, input.positionOS.xy);
    emissionFactor = clamp(1 - sign(dissolveFactor - _DissolveSpan), 0, 1);
    clip(dissolveFactor);
#endif
#if defined(DISTANCE_BASED_DISSOLVE)
    dissolveFactor = DissolveFactor_Distance_Noise(input.positionWS, _DistanceDissolveReferencePoint, input.positionWS.xy);
    //float dissolveFactor = distance(input.positionWS, _WorldSpaceCameraPos) - _DistanceDissolveEdge;
    emissionFactor = clamp(1 - sign(dissolveFactor - _DissolveSpan), 0, 1);
    clip(dissolveFactor);
    ...
    surfaceData.emission += _DissolveColor * emissionFactor;
