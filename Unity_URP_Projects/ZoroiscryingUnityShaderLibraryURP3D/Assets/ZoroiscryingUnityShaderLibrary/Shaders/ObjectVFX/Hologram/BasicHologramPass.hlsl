#ifndef BASIC_HOLOGRAM_PASS
#define BASIC_HOLOGRAM_PASS
...
Varyings LitPassVertex(Attributes input)
{
    ...
    input.positionOS.xyz += float3(value_noise11(_Time.x), value_noise11(_Time.y), 0) * (glitchStrength + distortionStrength);
    ...
    output.screenPos = vertexInput.positionVS.xy / vertexInput.positionVS.z;
    float multiplier = TransformWorldToView(TransformObjectToWorld(float3(0,0,0)));
    output.screenPos *= multiplier;
    output.screenPos = vertexInput.positionNDC.xy;
#endif
...
float ScanLine_Strength_SS(float2 positionSS, float3 scanDirection, half timeScale, half scanlineThickness, half scanlineSampleScale)
{
    float strength = 0.0;
    float scanVector = (dot(positionSS, scanDirection.xy) + 1)/2;
    //scanVector = (positionSS.y) / 2;
...
half4 LitPassFragment(Varyings input) : SV_Target
{
    ...
    float positionSSScale = TransformWorldToView(TransformObjectToWorld(float3(0,0,0))).z;
    float4 positionSS = input.positionSS * positionSSScale;
    float scanlineEmissionStrength1 =
    ScanLine_Strength_SS(input.screenPos, _ScanlineDirection1.xyz,
        _ScanlineSpeed1, _ScanlineThickness1, _ScanlineSampleScale1 * noiseV);
    float scanlineEmissionStrength2 =
    ScanLine_Strength_SS(input.screenPos, _ScanlineDirection2.xyz,
